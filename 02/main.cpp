...
static int count_num = 0;   //标志位，默认值为0代表程序当中只有主线程
pthread_mutex_t mutex;      //公共的锁
...
void Download_image(char *image_url)
{
    ...
    data->url = (char *)malloc(sizeof(char) * (len + 1));

    sprintf(data->filename, "%s", filename);
    sprintf(data->url, "%s", image_url);

    pthread_create(&ntid, NULL, Download_p, (void *)data);
